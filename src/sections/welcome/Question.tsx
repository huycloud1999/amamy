...
const Question = () => {
  return (
    <div className="w-full h-full bg-white flex justify-center items-center font-montserrat pt-[2.448rem] xsm:pt-[1.563rem] pb-[2.031rem] xsm:pb-[1.823rem]">
      <div className="w-full px-[6rem] xsm:px-[1rem]">
        <h2 className="font-bold text-[2.083rem] xsm:text-[1.25rem] xsm:text-center leading-[100%] tracking-normal pl-[0.417rem] xsm:pl-0 mb-[1.563rem] xsm:mb-[0.833rem]">Câu hỏi thường gặp</h2>
        <div className="grid grid-cols-12 xsm:grid-cols-1 gap-x-[1.458rem]">
          {/* Left */}
          <Accordion
            type="multiple"
            defaultValue={['item-0', 'item-1']}
            className="col-span-8 xsm:col-span-1 flex flex-col gap-[1.042rem] xsm:gap-[0.625rem]"
          >
            {faqs?.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="w-full rounded-[1.042rem] shadow-[0px_4.4px_20px_-1px_#1310220D]"
              >
                <AccordionTrigger
                  className="group flex justify-between items-center px-[1.667rem] xsm:px-[1.042rem] py-[1.458rem] xsm:py-[0.833rem] rounded-[1.25rem] border border-[#E3DBD8] hover:no-underline"
                  icon={
                    <svg className="transition-transform duration-200 text-[#38B6FF] size-[1.771rem] xsm:size-[1.042rem] group-data-[state=open]:text-[#cccccc]" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                >
                  <span className="xsm:w-[14.01rem] font-semibold xsm:font-medium text-[0.938rem] xsm:text-[0.729rem] leading-[1.458rem] tracking-normal">{faq.question}</span>
                </AccordionTrigger>
                <AccordionContent className="px-[1.458rem] xsm:px-[0.521rem] py-[0.938rem] xsm:pt-[0.833rem] xsm:pb-[0.625rem] font-normal text-[0.833rem] xsm:text-[0.729rem] leading-[1.563rem] xsm:leading-[1.25rem] tracking-normal text-[#727272] whitespace-pre-line">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
          ...
          {/* Right */}
          <div className="xsm:hidden col-span-4 xsm:col-span-1 flex flex-col gap-[1.042rem]">
            <Accordion
              type='single'
              collapsible
              className='flex flex-col rounded-[1.25rem] bg-[#F4FBFF] gap-[1.042rem] px-[1.979rem] pt-[2.396rem] pb-[1.563rem]'>
              {
                shippingRoutes.map((route, i) => (
                  <AccordionItem
                    ...
                    <AccordionTrigger
                      ...
                      className='flex justify-between items-center pb-[0.938rem] border-b border-[#E3DBD8] hover:text-Blue-Primary [&[data-state=open]]:text-Blue-Primary [&[data-state=open]>svg]:rotate-90 hover:no-underline'>
                      <span className='font-medium text-[1.25rem] leading-[100%] tracking-normal'>
                        {route.route}
                      </span>
                    </AccordionTrigger>
                    <AccordionContent>
            ...
            {/* Hotline */}
            <div className='flex flex-col items-center rounded-[1.25rem] bg-[#F4FBFF] px-[3.542rem] py-[2.083rem]'>
              <p className='font-bold text-[1.146rem] leading-[1.563rem] tracking-normal text-center'>Liên hệ tư vấn trực tiếp</p>
              <svg className='mt-[1.563rem] mb-[1.354rem]' width="96" height="96" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <p className='font-normal text-[0.833rem] leading-[1.563rem] text-center tracking-normal'>Hotline</p>
              <p className='font-bold text-[1.146rem] leading-[1.563rem] text-center tracking-normal'>092 677 79 66</p>
            </div>
          </div>

          {/* Hotline MB */}
          <div className='hidden xsm:flex col-span-1 flex-col gap-[1.25rem] mt-[1.823rem] w-full'>
            <div className='flex justify-between items-center pl-[1.771rem] pr-[1.146rem] py-[1.667rem] bg-[#F4FBFF] rounded-[1.25rem]'>
              <span className='text-[1.042rem] font-bold leading-[100%] tracking-normal'>Dịch vụ gửi hàng</span>
              <ChevronDown />
            </div>
            <div className='flex justify-between items-center pl-[1.146rem] pr-[1.458rem] py-[0.833rem] bg-[#F4FBFF] rounded-[1.25rem]'>
              <div className='flex justify-between'>
                ...
                <div className='flex flex-col gap-[0.234rem]'>
                  <p className='text-Blue-Primary text-[1.875rem] leading-[1.563rem] font-bold tracking-normal text-center'>092 677 79 66</p>
                  <p className='text-[1.094rem] leading-[1.563rem] font-bold tracking-normal text-center'>Liên hệ tư vấn trực tiếp</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <AccordionContent className='mt-[1.25rem] pb-0'>
            ...
            <div
              ...
              className=' text-black/80 text-pc-14 xsm:text-mb-12 whitespace-pre-line'
            ></div>
          </AccordionContent>
        </AccordionItem>
      ))}
