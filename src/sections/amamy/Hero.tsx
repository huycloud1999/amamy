...
export default function Hero() {
  useGSAP(() => {
    gsap.from('.fade-in-box-stats', {
      scrollTrigger: {
        trigger: '.fade-in-box-stats',
        start: 'top 80%',
      },
      opacity: 0,
      y: 50,
  ...
  return (
    <section className='bg-Blue-Primary text-white'>
      <div className='sm:px-[6rem] px-4 pb-14 pt-24 xsm:pb-9 xsm:pt-9'>
        <h1 className='fade-in-box text-center font-bold font-montserrat text-[4rem] xsm:text-2xl xsm:mb-4'>
          Bạn có ý định gửi hàng ?
        </h1>
        <p className='fade-in-box text-center font-montserrat text-base max-w-[41.875rem] xsm:max-w-[17.938rem] mx-auto mb-14 xsm:mb-7 xsm:text-sm'>
        ...
        <button
          type='button'
          className='fade-in-box bg-white text-Blue-Primary font-montserrat text-lg leading-6 font-medium px-6 py-3 rounded-3xl flex items-center gap-2 mx-auto mb-7 xsm:py-1.5 xsm:px-3 xsm:text-sm tracking-tight'
        >
          Dự tính giá gửi hàng qua Pháp
          <ArrowRight className='size-6 xsm:size-4' />
        </button>
        <hr className='fade-in-box border-white mb-7' />
        <div className='flex items-center justify-between xsm:items-start xsm:flex-col xsm:gap-8'>
          <div className='flex-1 fade-in-box-stats'>
            <h1 className='font-montserrat font-bold text-[2.5rem] xsm:text-2xl leading-none'>
              Xin chào
            </h1>
            <h2 className='font-montserrat font-bold text-5xl xsm:text-3xl leading-none whitespace-nowrap'>
              Chúng tôi là Amamy
            </h2>
          </div>
